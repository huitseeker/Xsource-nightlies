...
export _JAVA_OPTIONS="-XX:+UseConcMarkSweepGC -XX:-UseGCOverheadLimit"
# create classfiles - a higher version number does not work for Play,
# since with 2.10.4-RCs, dependencies have not been rebuilt
sbt-0.13.0 'set javacOptions += "-XX:+UseConcMarkSweepGC -XX:-UseGCOverheadLimit"' 'set every scalaVersion:="2.10.4"' compile
...
SCALA_211_DIR=scala-dist/target/universal/scala-2.11.1-SNAPSHOT
export PATH=`pwd`/$SCALA_211_DIR/bin:$PATH
cd scala-dist
sbt-0.13.0 'set version := "2.11.1-SNAPSHOT"' 'set resolvers += "private-repo" at "http://private-repo.typesafe.com/typesafe/scala-release-temp/"' universal:package-bin
cd target/universal && unzip scala-2.11.1-SNAPSHOT.zip
popd
...
#     http://www.scala-lang.org/files/archive/nightly/distributions/ |  perl -lne 'if (/scala-2.11.1[^\"]*?.zip/){print $&; exit}' )
